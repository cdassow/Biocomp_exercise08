...
guess <- function(){
  range <- c(1:100)
  randomnumber <- as.integer(sample(range, 1))
  counter <- 0
  playerguess <- 0 
  while(counter<10 && playerguess!=randomnumber){
    playerguess <- as.integer(readline(prompt = "guess a number between 1 and 100: "))
    if((playerguess-randomnumber) > 0){
      print("Lower")
      counter = counter+1
    }
    else if((playerguess-randomnumber) < 0){
      print("higher")
      counter = counter+1
    }  
    else if((playerguess-randomnumber) == 0){
